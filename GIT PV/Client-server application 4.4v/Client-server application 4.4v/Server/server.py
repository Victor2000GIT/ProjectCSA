    ...
    try:
        ...
        while True:
            try:
                ...
                if config.ClientCommand[0]:
                    print "OK"
                    _ClientCommand = config.ClientCommand[1]
                    config.ClientCommand = ("", 0)
                    if _ClientCommand == 1: # Cick
                        return
...
def CommandInterpreter():
    ...
    try:
        try:
            ...
            if CommandNum == 8: # Max clients
            ...
            elif CommandNum == 1: # Cick client
                if len(Command) > 1:
                    #Id = UserInBase(UsersCur, Command[1].capitalize()) # Получение ID пользователя из базы
                    config.ClientCommand = (str(Command[1]).capitalize(), 1)
            elif CommandNum == 0: # Set/unset debug for client
                if len(Command) > 1:
                    config.ClientCommand = (str(Command[1]).capitalize(), 0)
                ...
                if len(Command) > 1:
                    FilterQuest = []
            elif CommandNum == 3: #6 print filters
                print FilterQuest
...
##        return
        ...
        try:
            threading._start_new_thread(GenerateCaptchaCash, (1,)) # Проверка нового подключения
        except error, e:
            print ('error #%i: %s'%e.args)
            return
        print 'OK.'
